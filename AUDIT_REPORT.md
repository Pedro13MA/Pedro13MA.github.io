# AUDIT_REPORT.md — Limiar (Backend + Frontend)
**Modo:** Dry-run (diagnóstico sem correções aplicadas)  
**Data:** 2026-07-28  
**Repos:** `spotter-intelligence-hub` + `Pedro13MA.github.io`  
...
Os riscos atuais concentram-se em: (1) **score Limiar demasiado agressivo com PVPR**, (2) **homepage sem filtro `product_type=MAIN`**, (3) **taxonomia ACCESSORY demasiado larga**, (4) **perda de `discountPct` no mapeamento frontend**.

---

## 🔴 SEVERIDADE ALTA

### H1 — Score Limiar: PVPR sobrepõe histórico real e força “buy”
- **Ficheiro:** `spotter-intelligence-hub/src/api/limiar_index.py` (~L75–92, L157–160)
...
- **Ficheiro:** `spotter-intelligence-hub/src/api/repository.py` (~L889–944)  
- **Frontend:** `Pedro13MA.github.io/src/components/home/HomeLiveSections.tsx` (~L40)
...
- **Ficheiro:** `spotter-intelligence-hub/src/taxonomy/aliases.py` (~L104–112)  
...
- **Ficheiro:** `spotter-intelligence-hub/src/taxonomy/aliases.py` (~L92)
...
- **Ficheiro:** `Pedro13MA.github.io/src/lib/api.ts` (`summaryToProduct`, ~L208–233)
...
- **Ficheiro:** `spotter-intelligence-hub/src/api/repository.py` (~L919–925)
...
- **Ficheiro:** `Pedro13MA.github.io/src/components/product/OpportunityCard.tsx` (~L62–65)
- **Problema:** Mostra `PVPR 59,90€` em texto normal; não usa `line-through` (já usado em `StoreCompareTable.tsx`).
- **Solução:** Aplicar `line-through` ao PVPR e badge `−X%` separado do limiar.

### M2 — Sobreposição de badges no cartão
- **Ficheiro:** `OpportunityCard.tsx` (~L42–49)
- **Problema:** Até 3 badges (`semaphore` + `limiar` + `Mín. histórico`) no canto da imagem; em mobile podem empilhar sobre a foto.
...
### L7 — Testes de API limiar / schema seed
- **Ficheiro:** `tests/test_api_limiar.py`
...
| Score Limiar PVPR vs histórico | ⚠️ Funcional mas **agressivo demais** (H1) |
