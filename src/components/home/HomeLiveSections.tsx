...
type Props = {
  /** opportunities = destaque; secondary = esperar / quedas / cupões */
  variant?: "opportunities" | "secondary" | "all";
};

export function HomeLiveSections({ variant = "all" }: Props) {
  ...
  const showOpp = variant === "opportunities" || variant === "all";
  const showSecondary = variant === "secondary" || variant === "all";
          ...
          {error ? (
            <div className="mx-auto max-w-6xl px-4 pt-8 sm:px-6">
              <p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                Não foi possível carregar as oportunidades neste momento. Tenta novamente
                dentro de instantes.
              </p>
            </div>
          ) : null}
